...
else %is solverPars.readInitialStateFrom given?
...
if type == 1
    ...
    switch solverPars.solver
        case 1
            [fba.t,fba.y] = ode45(@(t,y) getDyWrapper(y,metabolicModel,reactor, solverPars, workerModels), [0:solverPars.timeStepSize:solverPars.tend], [reactor.biomassInit reactor. Init], options);
        case 2
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%
    [mb nb]=size(fba.y);
    for jb=1:length(metabolicModel)
        for ib=1:mb
            if result.biomass(ib,jb)<=0
                result.biomass(ib,jb)=0;
            end
        end
    end
    
    for jb=length(metabolicModel)+1:nb
        for ib=1:mb
            if result.compounds(ib,jb)<=0
                result.compounds(ib,jb)=0;
            end
        end
    end
    ...
    while result.time(n) < solverPars.tend
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [na ma]=size(result.biomass(n+1, :));
        
        for imm=1:ma
            if result.biomass(n+1, imm)<=0
                result.biomass(n+1, imm)=0;                
            end
            if result.biomass(n+1, imm)>=50
                result.biomass(n+1, imm)=50;                
            end
        end
        
          [na ma]=size(result.compounds(n+1, :));
        
        for imm=1:ma
            if result.compounds(n+1, imm)<=0
                result.compounds(n+1, imm)=0;                
            end
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if solveFullFBA == 1
            ...
            for i = 1:numberOfCompounds
                if result.compounds(n+1, i) < 0 && result.compounds(n, i) ~= 0  %%%%Eliminamos la condición de igualdad para compounds(n+1)
